...
class DetalleCobro extends Component{
    ...
    cargasDetail(){
        if (typeof (this.state.planDetails.cargas) != 'undefined' && this.state.planDetails.cargas.length > 0){
            return _.map(this.state.planDetails.cargas, carga => {
                return (
                    <Row fluid className="bg-light border-bottom py-1" key={"carga"+carga.age}>
                        <Col xs={5} sm={4} className="text-left text-muted">Carga {carga.age} Años</Col>
                        ...
                        <Col xs={1} sm={1} className="text-center text-muted">=</Col>
                        <Col xs={1} sm={1} className="text-center text-muted">{this.floatFormat.format(carga.finalPrice.toFixed(2))}</Col>
                    </Row>
                )
            })
        }
    ...
    render() {
        ...
        return(
            <React.Fragment>
                    <Row fluid className="bg-light border-bottom py-1">
                        ...
                        <Col xs={1} sm={1} className="text-right text-muted"></Col>
                        <Col xs={1} sm={1} className="font-weight-bold text-left text-muted">Valor final</Col>
                    </Row>
                    <Row fluid className="bg-light border-bottom py-1">
                        <Col xs={5} sm={4} className="text-left text-muted">Titular</Col>
                        ...
                        <Col xs={1} sm={1} className="text-center text-muted">{this.floatFormat.format(this.state.planDetails.GES)}</Col>
                        <Col xs={1} sm={1} className="text-center text-muted">=</Col>
                        <Col xs={1} sm={1} className="text-center text-muted">{this.floatFormat.format(this.state.planDetails.cotizante.finalPrice.toFixed(2))}</Col>
                    </Row>
                    {this.cargasDetail()}
                    <Row fluid className="bg-light  pt-3">
                        <Col xs={12} className="text-right font-weight-bold pr-2">
                            Total: {floatFormat.format(this.props.totalPrice.toFixed(2))}
                        </Col>
                    </Row>
                    <Row fluid className="bg-light py-3">
                        <Col xs={11}>
